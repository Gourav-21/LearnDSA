import CircularSkeleton from "@/components/Skeletons/CircleSkeleton";
...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem,_solved }) => {
  ...
  const { liked, disliked, starred, setData, solved } = useGetUsersDataOnProblem(
    problem.id
  );
	...
	const handleLike = async () => {
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				if (liked) {
					// remove problem id from likedProblems on user document, decrement like on problem document
					transaction.update(userRef, {
						likedProblems: userDoc.data().likedProblems.filter((id: string) => id !== problem.id),
					});
					transaction.update(problemRef, {
						like: problemDoc.data().like - 1,
					});

					setCurrentProblem((prev) => (prev ? { ...prev, like: prev.like - 1 } : null));
					...
					transaction.update(problemRef, {
						like: problemDoc.data().like + 1,
						dislike: problemDoc.data().dislike - 1,
					});

					setCurrentProblem((prev) =>
						prev ? { ...prev, like: prev.like + 1, dislike: prev.dislike - 1 } : null
					);
		...
		await runTransaction(firestore, async (transaction) => {
			...
			if (userDoc.exists() && problemDoc.exists()) {
				...
				if (disliked) {
					...
					setCurrentProblem((prev) => (prev ? { ...prev, dislike: prev.dislike - 1 } : null));
					...
					setCurrentProblem((prev) =>
						prev ? { ...prev, dislike: prev.dislike + 1, like: prev.like - 1 } : null
					);
  ...
  return (
    <div className="bg-dark-layer-1">
      ...
      <div className="flex h-11 w-full items-center pt-2 bg-dark-layer-2  text-black overflow-x-hidden">
        <div
          className={
            "bg-dark-layer-1 rounded-t-[5px] font-medium px-5 py-[10px] text-xs cursor-pointer"
          }
        >
          Description
        </div>
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                ...
                <div
                  ...
                  onClick={handleLike}
                >
                  {liked && !updating && <AiFillLike className="text-blue-300" />}
                ...
                <div onClick={handleDislike} className="flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px]  ml-4 text-lg transition-colors duration-200 text-green-s text-dark-gray-6">
                  {disliked && !updating && <AiFillDislike className="text-red-400" />}
